...
// Helper function to recursively calculate the depth of each node
const calculateDepth = (
  ...
  const depths = incomers.map(
    (incomer) => calculateDepth(incomer.id, nodes, edges, depthCache) + 1
  );
...
const d3HierarchyLayout: LayoutAlgorithm = async (nodes, edges, options) => {
  ...
  nodes.forEach((node) => calculateDepth(node.id, nodes, edges, depthCache));

  const getParentId = (node: Node) => {
    if (node.id === rootNode.id) return undefined;

    const incomers = getIncomers(node, nodes, edges);
    if (incomers.length === 0) return rootNode.id;

    // Sort incomers by their depth and select the one with the greatest depth
    const sortedIncomers = incomers.sort(
      (a, b) => depthCache.get(b.id)! - depthCache.get(a.id)!
    );
    return sortedIncomers[0].id;
  };
